...
namespace ModelPredictiveControl
{
	...
	{
		// Constants
		constexpr const f64 wheelBase  = 2.0;
		...
		BicycleModel<lookaheadN>::CreateOptions biSolverOptions;
		biSolverOptions = (BicycleModel<lookaheadN>::CreateOptions)((u8)BicycleModel<lookaheadN>::CreateOptions::PrecalculateAll | (u8)BicycleModel<lookaheadN>::CreateOptions::IgnoreUBounds);
		BicycleModel<lookaheadN> biSolver(biSolverOptions);
		biSolver.SetWheelBase(wheelBase);
		...
		for (u32 i = 0; i < ((sequenceN - 1) - lookaheadN); ++i)
		{
			...
			if (!BicycleModel<lookaheadN>::IsApprox(currentState, referenceStates[i + 1], (0.25 * 0.25), (0.1 * 0.1)))
			{
				return false;
			}
		}
	...
	bool SolverTests::TestConstrainedBicycleModel()
	{
		...
		// Create solver with precalculated flags and set the wheel radius
		BicycleModel<lookaheadN>::CreateOptions biSolverOptions;
		biSolverOptions = BicycleModel<lookaheadN>::CreateOptions::PrecalculateAll;
		BicycleModel<lookaheadN> biSolver(biSolverOptions);
		biSolver.SetWheelBase(wheelBase);
	...
	bool SolverTests::TestUnconstrainedQuadcopter()
	{
		...
		// Create solver with precalculated flags and set the wheel radius
		Quadcopter<lookaheadN>::CreateOptions quadSolverOptions;
		quadSolverOptions = (Quadcopter<lookaheadN>::CreateOptions)((u8)Quadcopter<lookaheadN>::CreateOptions::PrecalculateAll | (u8)Quadcopter<lookaheadN>::CreateOptions::IgnoreUBounds);
		Quadcopter<lookaheadN> quadSolver(quadSolverOptions);
		...
		for (u32 i = 0; i < ((sequenceN - 1) - lookaheadN); ++i)
		{
			...
			// If we never deviate (~0.5m-ish) from the path, then our MPC is working as expected because
			// we start from the same position
			if (!Quadcopter<lookaheadN>::IsApprox(currentState, referenceStates[i + 1], (0.25 * 0.25), (0.1 * 0.1)))
			{
				return false;
			}
		}
	...
	bool SolverTests::TestConstrainedQuadcopter()
	{
		...
		// Create solver with precalculated flags and set the wheel radius
		Quadcopter<lookaheadN>::CreateOptions quadSolverOptions;
		quadSolverOptions = Quadcopter<lookaheadN>::CreateOptions::PrecalculateAll;
		Quadcopter<lookaheadN> quadSolver(quadSolverOptions);
